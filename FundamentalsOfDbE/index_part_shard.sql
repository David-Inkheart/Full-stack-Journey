...
insert into grades_ori(g) select floor(random()*100) from generate_series(0, 10000000);
...
\d
select count(*) from grades_ori where g = 30;
explain analyze select count(*) from grades_ori where g = 30;
explain analyze select count(*) from grades_ori where g between 30 and 35;
...
insert into grades_parts select * from grades_ori;
